...
do
   ...
   cp initial.py set$i
   cp input_data_generate_f2.py set$i
   cd set$i/
   python3 initial.py > initial_conformation.txt
   python3 input_data_generate_f1.py > random_seed
   #nohup lmp_serial -in lammps_input_file.in &> sim$i.txt &
   #nohup python Pf_lammps.py &> hu_$i.txt &
   sbatch -c 1 --qos=normal --job-name=ELL_$i  --wrap='lmp_serial -in lammps_input_file.in &> chain.txt'
   cd ../
done
